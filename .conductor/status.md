...
- w3id redirect handoff is prepared in `docs/registry/w3id-submission.md` and `make w3id-packet`; release preflight now requires `dist/w3id-redirect-handoff.json`, and the release-assets workflow is configured to upload it. The external `perma-id/w3id.org` pull request has not been submitted.
...
- Release-assets workflow is configured to attach `w3id-redirect-handoff.json` to `v1.0.0`; dispatch and live asset verification remain open.
