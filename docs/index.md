# Documentation Index

## Repository Guides
- [Ontology design patterns](ontology-design-patterns.md)
- [Modelling decisions](modelling-decisions.md)
- [Glossary](glossary.md)
- [How to cite and reuse UOGTO](how-to-cite-and-reuse-uogto.md)
- [Release process](release-process.md)

## Program Tracking
- [RI-HERO overview](ri-hero.md)
- [RI-HERO status](ri-hero-status.md)

## Registry and Publication
- [arXiv submission process](paper/arxiv-submission-process.md)
- [arXiv submission contract](paper/arxiv-submission-contract.md)
- [arXiv post-submission provenance record template](paper/arxiv-post-submission-record-template.md)
- [LaTeX visual presentation scorecard](paper/latex-visual-presentation-scorecard.md)
- [LOV submission](registry/lov-submission.md)
- [OLS indexing](registry/ols-indexing.md)
- [w3id submission](registry/w3id-submission.md)
- [Extended discoverability submissions](registry/extended-discoverability-submissions.md)

## Comparative Research
- [Ontology comparison report](ontology-comparison/report.md)
- [Ontology comparison discovery protocol](ontology-comparison/discovery-protocol.md)
- [Repository-only raw search output supplement](article-hardening/raw-search-output-supplement.md)
- [Repository-only candidate decision ledger](article-hardening/candidate-decision-ledger.md)
- [Repository-only ontology snapshot supplement](article-hardening/ontology-snapshot-supplement.md)
- [Repository-only ontology citation register](article-hardening/ontology-citation-register.md)
- [Repository-only network graph visualisation supplement](article-hardening/network-graph-visualisation-supplement.md)
