...
def test_build_package_manifest_points_to_cleaned_stage_dir(tmp_path: Path) -> None:
    ...
    assert built_dir.name.startswith("arxiv-source-package-")
    assert (built_dir / "paper.tex").exists()
    assert not (built_dir / "notes.txt").exists()
    assert manifest["kept"] == ["paper.tex"]


def test_default_clean_resolves_latest_generated_manifest(tmp_path: Path, monkeypatch) -> None:
    default_dir = tmp_path / "arxiv-source-package"
    stale_default = default_dir
    stale_default.mkdir()
    (stale_default / "paper.tex").write_text("stale\n", encoding="utf-8")
